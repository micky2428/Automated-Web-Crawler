...
#翻頁功能
# for page in range(4, num_pages + 1):
#     # 更新 URL，將頁碼傳入參數"p"
...
for scroll in range(4):
    driver.execute_script('window.scrollBy(0,2000)') #106
    time.sleep(10)
    
product_url_soup = BeautifulSoup(driver.page_source, 'html.parser')
# product_url_tag = product_url_soup.find("h1", {"class": "name x-large-font"})
# product_links.append(product_url_tag)
...
for link in all_links:
    ...
    if "/site/sku/" in href:
        ...
        product_id = re.search(r'/sku/(\d+)', href)
